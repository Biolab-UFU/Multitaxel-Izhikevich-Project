...
#-------------------------------------------------------------------------------
Ui_MainWindow, QMainWindow = loadUiType('formAnalysisTexture_gui.ui')
#-------------------------------------------------------------------------------
class CONSTS():
    NOFILTER = 0
    HIGHPASS = 1
    ...
    SELECT_FORCE = 1
    SELECT_PALPATION = 2
class FormAnalysisTexture(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        super(FormAnalysisTexture,self).__init__()
        self.setupUi(self)
        self.setupGUI()
        #matplotlib
        #raw signal
...
#-------------------------------------------------------------------------------
    def doLoad(self):
        ...
        diag = QFileDialog()
        filename,_ = diag.getOpenFileName(None,'Title','','Texture data (*.txt)')
        #saves the poses
...
#-------------------------------------------------------------------------------
    def loadTactile(self):
        ...
        '''
        try:
            self.rawTactile = np.loadtxt(self.filename)
...
#-------------------------------------------------------------------------------
    def runIzhikevich(self):
        ...
        if self.chIzNormalize.isChecked():
            self.currents = [(x/np.max(x))*self.GF for x in self.procTactile]
        else: #otherwise, just multiply directly by GF
            self.currents = [x*self.GF for x in self.procTactile]
        #other parameters
        self.t0 = 0
